...
/**
 * 장식용 배경 버블. 원본 앱은 렌더 시점에 Math.random() 으로 만들었지만,
 * 그러면 서버/클라이언트 결과가 달라 하이드레이션이 어긋난다.
 * 고정 시드 PRNG 로 모듈 로드 시 한 번만 계산해 양쪽을 일치시킨다.
 */
const seededRandom = (() => {
...
const BACKGROUND_ELEMENTS: BackgroundElement[] = [...Array(10)].map(() => ({
  width: seededRandom() * 30 + 10,
  height: seededRandom() * 30 + 10,
  top: `${seededRandom() * 100}%`,
  left: `${seededRandom() * 100}%`,
  yAnimation: seededRandom() * -50 - 30,
  duration: seededRandom() * 8 + 10,
...
export default function Footer() {
  ...
  const backgroundElements = BACKGROUND_ELEMENTS;
